...
import { FilePenIcon, PlusIcon, RefreshCwIcon, TrashIcon } from "lucide-react"; // Import icons from lucide-react
...
export default function ExpenseTracker() {
  ...
  return (
    <div className="flex flex-col bg-yellow-100 h-screen">
      {/* Header section */}
      <header className="bg-primary bg-blue-800 text-primary-foreground py-4 px-6 shadow">
      ...
      <div className="fixed bottom-6 right-6">
        <Button
          size="icon"
          className="rounded-full bg-orange-700 shadow-lg"
          onClick={() => {
            setShowModal(true);
            setIsEditing(false);
            resetForm();
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="bg-card p-6 bg-orange-200 rounded-lg shadow w-full max-w-md">
          <DialogHeader>
            <DialogTitle className="text-orange-700 text-center">
          ...
          <DialogFooter>
            <Button className="bg-orange-300" variant="outline" onClick={() => setShowModal(false)}>
              Cancel
            </Button>
            <Button className="bg-orange-700 hover:bg-orange-950"
