...
export default function Home() {
  ...
  const renderForecast = () => {
    ...
    return dailyForecasts.map((day, index: number) => {
      ...
      return (
        <div key={index} className="text-center p-4 border text-blue-700 rounded-lg">
  ...
  return (
    <main
      style={{
        backgroundImage: `url('/weather-bg-img.jpg')`,
    ...
    >
      <div className="app w-[90%] sm:w-[60%] lg:w-[40%] bg-white border-4 rounded-lg border-gray-700 p-4">
        <h1 className="text-2xl font-bold mb-4 text-blue-900 text-center">Weather App</h1>
        <form onSubmit={fetchWeather} className="mb-4">
          <input
            ...
            className="p-2 border text-gray-600 rounded w-full"
          />
          <button
            type="submit"
            className="bg-blue-900 text-white p-2 rounded mt-2 w-full"
          >
            Get Weather
          </button>
        </form>
        ...
        {currentWeather && (
          <div className="current-weather flex items-center justify-around mb-6">
            ...
            <div className="flex flex-col text-red-700 items-center">
