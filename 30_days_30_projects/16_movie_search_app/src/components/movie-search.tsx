...
export default function MovieSearch() {
  ...
  const handleSearch = async (): Promise<void> => {
    ...
    try {
      const response = await fetch(
       `https://www.omdbapi.com/?t=${searchTerm}&apikey=a429119a`
        );
      if (!response.ok) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-blue-500 p-4">
      <div className="w-full max-w-lg p-6 bg-gray-300 rounded-lg shadow-md">
        ...
        {loading && (
          <div className="flex justify-center items-center">
            <ClipLoader className="w-6 h-6 text-purple-500" />
          </div>
        )}
        {/* Error message */}
        {error && (
